...
interface PostDetailModalProps {
  ...
  handleUpdateSlidePrompt?: (postIndex: number, slideIdx: number | null, prompt: string) => void;
...
export const PostDetailModal: React.FC<PostDetailModalProps> = ({
  ...
  // Exit prompt edit mode when navigating to another slide
  useEffect(() => {
    setEditingPrompt(false);
    setPromptDraft('');
  }, [slideIdx]);
  ...
  const handleSavePromptEdit = () => {
    if (!handleUpdateSlidePrompt) return;
    handleUpdateSlidePrompt(postIndex, currentSlide ? slideIdx : null, promptDraft.trim());
    setEditingPrompt(false);
    setPromptDraft('');
  };
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <div className="flex items-center gap-2">
                        {!editingPrompt && handleUpdateSlidePrompt && (
                          <button
                            type="button"
                            onClick={() => { setPromptDraft(displayPrompt || ''); setEditingPrompt(true); }}
                    ...
                    {editingPrompt ? (
                      <div className="space-y-2">
                        <textarea
                          ...
                          className="w-full px-3 py-2 bg-white dark:bg-slate-800 text-slate-900 dark:text-white border border-purple-500/40 rounded-xl text-xs outline-none font-mono"
                        />
                        <div className="flex gap-2 justify-end">
                          <button
                            type="button"
                            onClick={() => { setEditingPrompt(false); setPromptDraft(''); }}
                            className="px-3 py-1.5 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-xs font-semibold rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer"
                          >
                            Cancel
                          </button>
                          <button
                            ...
                            className="px-3 py-1.5 bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold rounded-lg disabled:opacity-50 cursor-pointer"
                          >
                            Save Prompt
                          </button>
                        </div>
